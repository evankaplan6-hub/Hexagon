...
`~/Hexagon` is the main folder for all Hexagon work. The repo lived at `~/Codex/hexagon`
until 2026-09-10; it was moved here with its full git history. There is no other Hexagon
checkout — if you find a reference to `~/Codex/hexagon` anywhere, it is stale.
...
- Branch `main`. A second branch `worktree-hexagon-research` is checked out as a git worktree at
  `.Codex/worktrees/hexagon-research` (locked, gitignored).
