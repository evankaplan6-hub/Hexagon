...
const OUT = flag('out', 'data/lab/markets.jsonl');
...
(async () => {
  ...
  const series = Object.entries(busy).filter(([, n]) => n >= SERIES_MIN_BUSY).map(([s]) => s).sort();
  ...
  const universe = await cached(path.join(DIR, 'universe.json'), async () => {
    const out = [];
    let done = 0;
    await pool(series, 4, async (s) => {
      ...
      try {
        do {
          const d = await get(`${BASE}/markets?series_ticker=${s}&status=settled&min_close_ts=${SINCE}&limit=1000${cursor ? `&cursor=${cursor}` : ''}`);
          for (const m of d.markets || []) {
            ...
            if (!(closeTs >= SINCE) || !schedEnd || schedEnd - openTs < MIN_LIFE_H * 3600) continue;
  ...
  // 3. fee multiplier and category per series (0 on some Politics/Crypto series, 0.5 on MLB games)
  const seriesInfo = new Map();
  await pool([...new Set(picked.map((m) => m.series))], 4, async (s) => {
  ...
  for (let i = 0; i < byClose.length; i += 5) batches.push(byClose.slice(i, i + 5));
  ...
  await pool(batches, 4, async (batch) => {
    ...
    try {
      const d = await get(`${BASE}/markets/candlesticks?market_tickers=${batch.map((m) => m.ticker).join(',')}&start_ts=${start}&end_ts=${end}&period_interval=60`);
      ...
      for (const m of batch) {
        ...
        for (const k of byTicker.get(m.ticker) || []) {
          ...
          const b = k.yes_bid && cents(k.yes_bid.close_dollars), a = k.yes_ask && cents(k.yes_ask.close_dollars);
          if (!(b > 0 && a < 100 && a > b)) continue;
          rows.push([Math.floor(k.end_period_ts / 3600) * 3600, b, a, Math.round(parseFloat(k.volume_fp) || 0)]);
        }
