...
const { planPull, listDir, isBytePrefix, parseArgs, addDays, run } = pull;
const { planTapeTrim, makeDiskBrake } = require('../src/recorder');
...
// 12:00 in New York on the 14th; the only clock this suite uses
...
group('listDir: sizes and hashes of closed days, today listed but not hashed');
{
  ...
  const r = listDir({ dir, todayET: TODAY });
  ...
  ok('`only` limits what gets read', listDir({ dir, todayET: TODAY, only: ['journal-2026-09-12.jsonl'] }).files.length === 1);
  ok('a folder that does not exist yet lists as empty', listDir({ dir: path.join(tmpRoot, 'nope'), todayET: TODAY }).files.length === 0);
  // the function is shipped to the box as source text: it must survive that on its own
  const shipped = eval(`(${listDir.toString()})`);
  ok('listDir runs from its own source text, with no closure', shipped({ dir, todayET: TODAY }).files.length === r.files.length);
}
...
function main() {
  if (a[0] === 'ssh' && a[1] === 'console') {
    ...
    if (m) {
      const src = Buffer.from(m[1], 'base64').toString();
      ...
      eval(src.replace('{"dir":"/data"', '{"dir":' + JSON.stringify(box)));
      return 0;
    }
...
function scene(label) {
  ...
  const lines = [];
  const go = (argv, env = {}) => {
    const keys = ['FAKE_BOX', 'FAKE_CALLS', 'FAKE_FAIL_LIST', 'FAKE_FAIL_GET', 'FAKE_CORRUPT_GET', 'FAKE_NO_MACHINE'];
    const saved = Object.fromEntries(keys.map((k) => [k, process.env[k]]));
    Object.assign(process.env, { FAKE_BOX: box, FAKE_CALLS: calls }, env);
    try { fs.rmSync(calls, { force: true }); lines.length = 0; return run([...argv, '--dest', dest], { now: () => new Date(NOW), fly: [process.execPath, fakeFly], cwd: root, out: (s) => lines.push(s), err: (s) => lines.push(s) }); }
...
group('run: guards');
{
  ...
  const lines = [];
  const code = run(['--dest', path.join(s.root, 'data', 'fly')], { now: () => new Date(NOW), fly: [process.execPath, fakeFly], cwd: s.root, out: (x) => lines.push(x), err: (x) => lines.push(x) });
  ...
  const code2 = run(['--keep', '0'], { now: () => new Date(NOW), fly: [process.execPath, fakeFly], cwd: s.root, out: () => {}, err: () => {} });
...
function fakeDisk(sizes, freeMb) {
  const d = { files: { ...sizes }, free: freeMb * MB, statfs: 0, readdir: 0, unlinked: [], failStatfs: false, failUnlink: null, missing: new Set() };
  d.io = {
    statfsSync: () => { d.statfs++; if (d.failStatfs) throw new Error('ENOSYS'); return { bavail: d.free, bsize: 1 }; },
    readdirSync: () => { d.readdir++; return Object.keys(d.files); },
    ...
    unlinkSync: (p) => {
      ...
      if (d.missing.has(n)) { const e = new Error('ENOENT'); e.code = 'ENOENT'; delete d.files[n]; throw e; }
      d.free += d.files[n]; delete d.files[n]; d.unlinked.push(n);
    },
  };
  return d;
}
...
group('the brake: never throws into the cycle');
{
  ...
  const d3 = fakeDisk({ 'ticks-2026-09-10.jsonl': 30 * MB, 'ticks-2026-09-11.jsonl': 300 * MB }, 150);
  d3.missing.add('ticks-2026-09-10.jsonl');   // the pull deleted it a moment ago
  const E3 = fakeE();
  makeDiskBrake(cfg, d3.io)(E3, NOW);
  ok('a tape already gone is skipped, and the next oldest goes instead', JSON.stringify(d3.unlinked) === '["ticks-2026-09-11.jsonl"]', d3.unlinked);
