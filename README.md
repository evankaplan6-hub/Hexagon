...
The floor at `/` is a wall of boards, one screen each, laid out for reading (nothing under 12px). Since 2026-09-25 it has no pixel room and no bots drawn at desks: the headline (every book against simply holding what it holds, today, fees, cash at work, and one sentence on why the two differ), the desk (the stock market's hours, how fresh each price is, how much of the daily loss limit is used, and the prediction-market desk winding down), one card per book (its P&L against holding, its own line, every market it trades with its price, today's change, how hard it swings and how much the book wants, and when it checks next; a book that has never traded is just its figure, its markets and its next check), the P&L chart against holding (the change over the range shown, the line broken where the desk was down, the axis in Eastern time with each day marked at Eastern midnight, every minute of the last day and the days before thinned to about 1,500 points), and what the bots are doing, with the routine rounds counted and hidden until asked for. The light in the header says Working, Waiting on prices (crypto quotes stopped), Not buying (past the daily loss limit, amber: the desk still marks and sells), Stalled (no round has finished for two minutes, though the server still answers: every figure then stops where it was; at five minutes the desk's watchdog saves, says so in both journals and ends the process, and Fly starts it again, as the prediction-market desk's loops always have, except in live mode) or No signal (the page has heard nothing for 12 seconds). A trade on that list comes from the ledger's own fills, and the log keeps only its newest 60 routine lines (thirteen an hour), so the day's trades and decisions stay on it: until 2026-09-26 one ring of 400 lines let the routine rounds push them out of the page within half a day. Out of hours the page says so: SPY "closed" rather than "15 min late", its last trade and the last session's move (Cboe's previous close turns into the day's own close after the bell, so SPY read 0.00% all weekend), and the options book's last 12:30 test under its weekday, with when the next one is. On a screen under 1,000px tall the chart and the log come before the books. A tablet gets two columns; a phone gets the headline, each book as its figure (tap its chip to open it), the latest five events, the chart, then the desk.
