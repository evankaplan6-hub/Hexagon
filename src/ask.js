...
- Six desks work the trading floor. HOLT matches the same outcome across the two venues. ILSA reads which way prices are drifting, and runs whale watch: it announces big bets by top wallets on Polymarket's leaderboards (sports, politics, economics, crypto and more; the whale_bets tool says which) but never trades on them. TESS watches health and risk and halts new trades on stale data, API errors or a bad day. RIGO marks open positions, closes them and settles finished markets. BRAM prices each matched pair and decides whether a trade is worth taking. KETT places the trades.
